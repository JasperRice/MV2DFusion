...
class Voxel2PointScatterNeck(nn.Module):
    ...
    def forward(
        self, points, pts_coors, voxel_feats, voxel2point_inds, voxel_padding=-1
    ):
        """Forward function.
        ...
        pts_feats = voxel_feats[
            voxel2point_inds
        ]  # voxel_feats must be the output of torch_scatter, voxel2point_inds is the input of torch_scatter
        pts_mask = ~(
            (pts_feats == voxel_padding).all(1)
        )  # some dropped voxels are padded
        if self.with_xyz:
            ...
            voxel_size = torch.tensor(
                self.voxel_size, dtype=dtype, device=device
            ).reshape(1, 3)
            pc_min_range = torch.tensor(
                self.point_cloud_range[:3], dtype=dtype, device=device
            ).reshape(1, 3)
            voxel_center_each_pts = (
                pts_coors[:, [3, 2, 1]].to(dtype).to(device) + 0.5
            ) * voxel_size + pc_min_range  # x y z order
