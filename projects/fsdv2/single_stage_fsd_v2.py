...
from .ops.sst.sst_ops import build_mlp, get_inner_win_inds, scatter_v2
...
class SingleStageFSDV2(SingleStage3DDetector):
    ...
    def __init__(
        ...
        self.virtual_proj = build_mlp(
            vpp["in_channels"], vpp["hidden_dims"], vpp["norm_cfg"]
        )
        self.ori_proj = build_mlp(
            vpp["ori_in_channels"], vpp["ori_hidden_dims"], vpp["norm_cfg"]
        )
        self.zero_virtual_feature = vpp.get("zero_virtual_feature", False)
        self.only_virtual = vpp.get("only_virtual", False)

        if self.as_rpn:
            self.recover_proj = build_mlp(
                vpp["recover_in_channels"], vpp["recover_hidden_dims"], vpp["norm_cfg"]
            )
        ...
        if multiscale_cfg is not None:
            ...
            for proj in multiscale_cfg["projector_hiddens"]:
                this_projector = build_mlp(
                    proj[0], proj[1:], multiscale_cfg["norm_cfg"]
                )
    ...
    def recover_point_features(
        ...
        if is_same:
            ...
            voxel_size = torch.tensor(
                self.virtual_voxel_size, device=device
            )  # correct only if is_same
            pc_range = torch.tensor(self.point_cloud_range, device=device)

            voxel_coors_per_pts = out_coors[voxel_encoder_inv]
            feat_per_pts = out_voxel_feats[voxel_encoder_inv]
            voxel_center_per_pts = (
                voxel_coors_per_pts[:, [3, 2, 1]] + 0.5
            ) * voxel_size[None, :] + pc_range[None, :3]
    ...
    def extract_feat(
        self, sampled_dict, origin_dict, gt_bboxes_3d=None, multiscale_features=None
    ):
        """Extract features from points."""
        if self.baseline_mode:
            return self.extract_feat_baseline(
                sampled_dict, origin_dict, gt_bboxes_3d, multiscale_features
            )
        ...
        proj_input = torch.cat(
            [sampled_feats, offset, sampled_logits, sampled_pts[:, 3:]], 1
        )
        ...
        voxel_feats, voxel_coors, unq_inv = self.voxel_encoder(
            voxel_encoder_input, coors, return_inv=True
        )
        ...
        voxel_indicators, scatter_coors = scatter_v2(
            pts_indicators, coors, mode="avg", return_inv=False
        )
        ...
        if multiscale_features is not None:
            voxel_feats, voxel_coors, singlescale_mask = self.multiscale_fusion(
                multiscale_features, voxel_feats, voxel_coors
            )
        ...
        if multiscale_features is not None:
            ...
            voxel_coors = voxel_coors[
                singlescale_mask
            ]  # in fact, out_coors and voxel_coors are same
        ...
        pc_range = torch.tensor(self.point_cloud_range, device=device)
        voxel_centers = (out_coors[:, [3, 2, 1]] + 0.5) * voxel_size[
            None, :
        ] + pc_range[None, :3]
        ...
        if self.training:
            self.print_info["num_virtual"] = virtual_voxel_feats.new_ones(1) * len(
                virtual_voxel_feats
            )

            alpha = self.train_cfg.get("centroid_alpha", None)
            if alpha is not None:
                gt_fg_mask = self.get_batched_gt_fg_mask(
                    cat_pts[:, :3], cat_batch_idx, gt_bboxes_3d
                )
                alpha_mask = (~gt_fg_mask).float() * alpha + gt_fg_mask.float()
                sum_centroid, _ = scatter_v2(
                    alpha_mask[:, None] * cat_pts[:, :3],
                    coors,
                    mode="sum",
                    return_inv=False,
                )
                sum_alpha, _ = scatter_v2(
                    alpha_mask[:, None], coors, mode="sum", return_inv=False
                )
                assert (sum_alpha >= alpha).all()
                voxel_centroid = sum_centroid / sum_alpha
            else:
                voxel_centroid, _ = scatter_v2(
                    cat_pts[:, :3], coors, mode="avg", return_inv=False
                )

            voxel_centroid = voxel_centroid[virtual_mask]
            out_dict["virtual_centroid"] = voxel_centroid
            assert (
                (voxel_centroid - virtual_centers).abs() < voxel_size / 2 + 1e-3
            ).all()

        if self.as_rpn:
            # need pts information for GroupCorrection
            out_pts_feats = self.recover_point_features(
                out_voxel_feats,
                out_coors,
                sparse_shape,
                cat_pts,
    ...
    def multiscale_fusion(self, ms_data, voxel_feats, coors):
        ...
        ms_feats = [
            self.ms_projectors[i](ms_data[i].features) for i in range(len(ms_data))
        ]
        ms_coors = [
            self.ms_coors_proj(data.indices, data.spatial_shape) for data in ms_data
        ]
        ...
        indicators = torch.cat(
            [
                voxel_feats.new_ones(len(voxel_feats), 1),
                voxel_feats.new_zeros(num_add_feats, 1),
            ],
            0,
        )

        out_feats, out_coors = scatter_v2(
            cat_feats, cat_coors, mode=cfg["fusion_mode"], return_inv=False
        )
        out_indicators, _ = scatter_v2(
            indicators, cat_coors, mode="max", return_inv=False
        )
    ...
    def forward_train(
        ...
        extract_output = self.extract_feat(
            combined_out,
            dict_to_sample,
            gt_bboxes_3d=gt_bboxes_3d,
            multiscale_features=seg_out_dict["decoder_features"],
        )
        ...
        det_loss = self.bbox_head.loss(
            *loss_inputs,
            ...
            aux_xyz=extract_output["virtual_centroid"]
        )
        ...
        query_pred = [
            torch.cat([x[0].tensor[:, 3:], x[1][:, None]], dim=1) for x in bbox_list
        ]
    ...
    def group_sample(self, dict_to_sample, offset):
        ...
        assert (
            bsz == 1
        ), "Maybe some codes need to be modified if bsz > 1, this will be updated very soon"
