...
class SparseClusterHead(BaseModule):
    ...
    def __init__(
        ...
        self.sync_reg_avg_factor = (
            False if train_cfg is None else train_cfg.get("sync_reg_avg_factor", True)
        )
        self.sync_cls_avg_factor = (
            False if train_cfg is None else train_cfg.get("sync_cls_avg_factor", False)
        )
        self.as_rpn = as_rpn
        ...
        if len(shared_mlp_dims) > 0:
            self.shared_mlp = build_mlp(
                in_channel, shared_mlp_dims, norm_cfg, act=act, dropout=shared_dropout
            )
        ...
        if reg_mlp is not None:
            self.conv_reg = build_mlp(
                ...
                reg_mlp
                + [
                    self.box_code_size * num_anchors,
                ],
                norm_cfg,
                True,
                act=act,
    ...
    def loss(
        ...
        targets = self.get_targets(
            cluster_xyz, cluster_batch_idx, gt_bboxes_3d, gt_labels_3d, reg_preds
        )
        ...
        if num_pos > 0:
            ...
            if code_weight:
                pos_bbox_weights = (
                    pos_bbox_weights * bbox_weights.new_tensor(code_weight)[None, :]
                )
        ...
        if self.corner_loss_cfg is not None:
            losses["loss_corner"] = self.get_corner_loss(
                pos_reg_preds, pos_bbox_targets, cluster_xyz[pos_inds], reg_avg_factor
            )
    ...
    def get_corner_loss(self, reg_preds, bbox_targets, base_points, reg_avg_factor):
        ...
        dets = self.bbox_coder.decode(
            reg_preds, base_points, self.corner_loss_cfg.get("detach_yaw", True)
        )
        gts = self.bbox_coder.decode(bbox_targets, base_points)
        corner_loss = self.corner_loss(dets, gts, self.corner_loss_cfg["delta"]).sum()
        corner_loss = (
            corner_loss.sum() / reg_avg_factor * self.corner_loss_cfg["loss_weight"]
        )
        return corner_loss
    ...
    def get_targets(
        self, cluster_xyz, batch_idx, gt_bboxes_3d, gt_labels_3d, reg_preds=None
    ):
    ...
    def get_targets_single(
        self, cluster_xyz, gt_bboxes_3d, gt_labels_3d, reg_preds=None
    ):
        ...
        if self.train_cfg.get("assign_by_dist", False):
            assign_result = self.assign_by_dist_single(
                cluster_xyz, gt_bboxes_3d, gt_labels_3d
            )
        else:
        ...
        sample_result = self.sampler.sample(
            assign_result, cluster_xyz, gt_bboxes_3d.tensor
        )  # Pseudo Sampler, use cluster_xyz as pseudo bbox here.
        ...
        labels = gt_labels_3d.new_full(
            (num_cluster,), self.num_classes, dtype=torch.long
        )
        ...
        bbox_targets[pos_inds] = self.bbox_coder.encode(
            sample_result.pos_gt_bboxes, cluster_xyz[pos_inds]
        )

        if self.loss_iou is not None:
            iou_labels = self.get_iou_labels(
                reg_preds, cluster_xyz, gt_bboxes_3d.tensor, pos_inds
            )
        else:
            iou_labels = None
    ...
    def get_iou_labels(self, reg_preds, cluster_xyz, gt_bboxes_3d, pos_inds):
        ...
        ious = bbox_overlaps_3d(
            bbox_preds, gt_bboxes_3d, mode="iou", coordinate="lidar"
        )  # [num_preds, num_gts]
        ious = ious.max(1)[0]
        ...
        iou_labels[interval_mask] = (ious[interval_mask] - iou_bg_thresh) / (
    ...
    def assign_single(
        ...
        assigned_gt_inds = cluster_xyz.new_zeros(
            (num_cluster,), dtype=torch.long
        )  # 0 indicates assign to backgroud
        ...
        bboxes7dim = enlarged_box.__class__(enlarged_box.tensor[..., :7])
        inbox_inds = bboxes7dim.points_in_boxes(cluster_xyz).long()
        # inbox_inds = enlarged_box.points_in_boxes(cluster_xyz).long()
        inbox_inds = self.dist_constrain(
            inbox_inds, cluster_xyz, gt_bboxes_3d, gt_labels_3d
        )
        pos_cluster_mask = inbox_inds > -1

        if pos_cluster_mask.any():
            assigned_gt_inds[pos_cluster_mask] = inbox_inds[pos_cluster_mask] + 1
            assigned_labels[pos_cluster_mask] = gt_labels_3d[
                inbox_inds[pos_cluster_mask]
            ]

        return AssignResult(num_gts, assigned_gt_inds, None, labels=assigned_labels)

    def assign_by_dist_single(
        self,
        cluster_xyz,
        gt_bboxes_3d,
        gt_labels_3d,
        ...
        num_matched_gt = torch.tensor(
            num_matched_gt, dtype=torch.float, device=cluster_xyz.device
        )
        ...
        if pos_cluster_mask.any():
            ...
            assigned_labels[pos_cluster_mask] = gt_labels_3d[
                matched_gt_inds[pos_cluster_mask]
            ]
    ...
    def dist_constrain(self, inbox_inds, cluster_xyz, gt_bboxes_3d, gt_labels_3d):
        ...
        rel_dist = torch.linalg.norm(
            pos_xyz[:, :2] - pos_box_center[:, :2], ord=2, dim=1
        )  # only xy-dist
    ...
    def _get_bboxes_single(
        ...
        results = box3d_multiclass_nms(
            bboxes, bboxes_for_nms, scores, score_thr, cfg.max_num, cfg
        )
