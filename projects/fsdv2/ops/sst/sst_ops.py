...
def flat2window(feat, voxel_drop_lvl, flat2win_inds_dict, drop_info, padding=0):
    ...
    for dl in drop_info:
        ...
        feat_3d = (
            torch.ones((num_windows * max_tokens, feat_dim), dtype=dtype, device=device)
            * padding
        )
...
def flat2window_v2(feat, inds_dict, padding=0):
    assert (
        "voxel_drop_level" in inds_dict
    ), "voxel_drop_level should be in inds_dict in v2 function"
    inds_v1 = {k: inds_dict[k] for k in inds_dict if not isinstance(k, str)}
    batching_info = inds_dict["batching_info"]
    return flat2window(
        feat, inds_dict["voxel_drop_level"], inds_v1, batching_info, padding=padding
    )
...
def scatter_v2(
    ...
    if unq_inv is None and min_points > 0:
        ...
        assert (
            new_coors is not None
        ), "please pass new_coors for interface consistency, caller: {}".format(
...
def filter_almost_empty(pts_coors, min_points=5):
    if min_points > 0:
        ...
        valid_mask = torch.ones(
            len(pts_coors), device=pts_coors.device, dtype=torch.bool
        )
    return valid_mask
...
def build_mlp(
    in_channel, hidden_dims, norm_cfg, is_head=False, act="relu", bias=False, dropout=0
):
    layer_list = []
    ...
    for i, c in enumerate(hidden_dims):
        ...
        if i == len(hidden_dims) - 1 and is_head:
            layer_list.append(
                nn.Linear(last_channel, c, bias=True),
            )
        else:
            sq = [
