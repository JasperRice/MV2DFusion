...
from mmdet3d.models import (build_backbone, build_detector, build_head,
                            build_neck, builder)
...
from .ops.sst.sst_ops import get_inner_win_inds, scatter_v2
...
def filter_almost_empty(coors, min_points):
    new_coors, unq_inv, unq_cnt = torch.unique(
        coors, return_inverse=True, return_counts=True, dim=0
    )
...
def find_connected_componets(points, batch_idx, dist):
    ...
    for i in range(bsz):
        ...
        if batch_mask.any():
            ...
            dist_mat = (
                this_points[:, None, :2] - this_points[None, :, :2]
            )  # only care about xy
...
def ssg_single_sample(points, num_fps, radius):
    ...
    triangle1 = torch.arange(len(key_points))[None, :].expand(
        len(key_points), -1
    )  # [[0,1,2], [0, 1, 2]]
    ...
    valid_centers_mask = (
        in_radius_mask.sum(0) == 1
    )  # if a point falls into multiple balls or does not fall into any ball, it is invalid.
...
class VoteSegmentor(Base3DSegmentor):
    ...
    def extract_feat(self, points, img_metas):
        ...
        coors = coors.long()
        voxel_features, voxel_coors, voxel2point_inds = self.voxel_encoder(
            batch_points, coors, return_inv=True
        )
        ...
        if "shuffle_inds" not in voxel_info:
            ...
            voxel_feats_reorder = self.reorder(
                x["voxel_feats"],
                voxel_info["shuffle_inds"],
                voxel_info["voxel_keep_inds"],
                padding,
            )  #'not consistent with voxel_coors any more'

        out = self.decode_neck(
            batch_points, coors, voxel_feats_reorder, voxel2point_inds, padding
        )
    ...
    def voxel_downsample(self, points_list):
        ...
        for points in points_list:
            ...
            out_points, new_coors = scatter_v2(
                points, coors, mode="avg", return_inv=False
            )
    ...
    def forward_train(
        ...
        if self.tanh_dims is not None:
            ...
            points = [
                torch.cat([p[:, :3], torch.tanh(p[:, 3:])], dim=1) for p in points
            ]

        if self.voxel_downsampling_size is not None:
            points = self.voxel_downsample(points)

        labels, vote_targets, vote_mask = self.segmentation_head.get_targets(
            points, gt_bboxes_3d, gt_labels_3d
        )

        if self.use_multiscale_features:
            neck_out, pts_coors, points, decoder_features = self.extract_feat(
                points, img_metas
            )
        else:
        ...
        if as_subsegmentor:
            loss_decode, preds_dict = self.segmentation_head.forward_train(
                feats, img_metas, labels, vote_targets, vote_mask, return_preds=True
            )
            ...
            loss_decode = self.segmentation_head.forward_train(
                feats, img_metas, labels, vote_targets, vote_mask, return_preds=False
            )
            losses.update(loss_decode)
            output_dict = losses
    ...
    @force_fp32(apply_to="points")
    def simple_test(
        self, points, img_metas, gt_bboxes_3d=None, gt_labels_3d=None, rescale=False
    ):
        ...
        if self.use_multiscale_features:
            x, pts_coors, points, decoder_features = self.extract_feat(
                points, img_metas
            )
        else:
        ...
        seg_logits, vote_preds = self.segmentation_head.forward_test(
            feats, img_metas, self.test_cfg
        )
...
class SingleStageFSD(SingleStage3DDetector):
    ...
    def extract_feat(
        self, points, pts_feats, pts_cluster_inds, img_metas, center_preds
    ):
        """Extract features from points."""
        cluster_xyz, _, inv_inds = scatter_v2(
            center_preds, pts_cluster_inds, mode="avg", return_inv=True
        )

        f_cluster = points[:, :3] - cluster_xyz[inv_inds]

        out_pts_feats, cluster_feats, out_coors = self.backbone(
            points, pts_feats, pts_cluster_inds, f_cluster
        )
        out_dict = dict(
            cluster_feats=cluster_feats, cluster_xyz=cluster_xyz, cluster_inds=out_coors
        )
        if self.as_rpn:
    ...
    def forward_train(
        ...
        sampled_out = self.sample(
            dict_to_sample, dict_to_sample["vote_offsets"], gt_bboxes_3d, gt_labels_3d
        )  # per cls list in sampled_out

        # we filter almost empty voxel in clustering, so here is a valid_mask
        cluster_inds_list, valid_mask_list = self.cluster_assigner(
            sampled_out["center_preds"],
            sampled_out["batch_idx"],
            gt_bboxes_3d,
            gt_labels_3d,
            origin_points=sampled_out["seg_points"],
        )  # per cls list
        pts_cluster_inds = torch.cat(
            cluster_inds_list, dim=0
        )  # [N, 3], (cls_id, batch_idx, cluster_id)

        num_clusters = (
            len(torch.unique(pts_cluster_inds, dim=0))
            * torch.ones((1,), device=pts_cluster_inds.device).float()
        )
        ...
        combined_out = self.combine_classes(
            sampled_out,
            ["seg_points", "seg_logits", "seg_vote_preds", "seg_feats", "center_preds"],
        )

        points = combined_out["seg_points"]
        pts_feats = torch.cat(
            [
                combined_out["seg_logits"],
                combined_out["seg_vote_preds"],
                combined_out["seg_feats"],
            ],
            dim=1,
        )
        assert len(pts_cluster_inds) == len(points) == len(pts_feats)
        losses["num_fg_points"] = torch.ones((1,), device=points.device).float() * len(
            points
        )

        extracted_outs = self.extract_feat(
            points, pts_feats, pts_cluster_inds, img_metas, combined_out["center_preds"]
        )
        ...
        for k in sampled_out:
            ...
            if len(old_data[0]) == len(valid_mask_list[0]) or "fg_mask" in k:
                if "fg_mask" in k:
                    ...
                    new_data_list = [
                        data[mask] for data, mask in zip(old_data, valid_mask_list)
                    ]
                    sampled_out[k] = new_data_list
        return sampled_out
    ...
    def pre_voxelize(self, data_dict):
        ...
        pc_range = torch.tensor(
            self.cluster_assigner.point_cloud_range, device=points.device
        )
        coors = torch.div(
            points[:, :3] - pc_range[None, :3],
            voxel_size[None, :],
    ...
    def simple_test(
        ...
        if gt_bboxes_3d is not None:
            ...
            assert (
                len(gt_bboxes_3d) == len(gt_labels_3d) == 1
            ), "assuming single sample testing"
            gt_bboxes_3d = [b[l >= 0] for b, l in zip(gt_bboxes_3d, gt_labels_3d)]
            gt_labels_3d = [l[l >= 0] for l in gt_labels_3d]
    ...
    def sample(self, dict_to_sample, offset, gt_bboxes_3d=None, gt_labels_3d=None):
        ...
        for cls in range(self.num_classes):
            ...
            fg_mask = self.get_fg_mask(
                seg_scores, seg_points, cls, batch_idx, gt_bboxes_3d, gt_labels_3d
            )

            if len(torch.unique(batch_idx[fg_mask])) < batch_size:
                one_random_pos_per_sample = self.get_sample_beg_position(
                    batch_idx, fg_mask
                )
                fg_mask[one_random_pos_per_sample] = (
                    True  # at least one point per sample
                )
    ...
    def get_fg_mask(
        self, seg_scores, seg_points, cls_id, batch_inds, gt_bboxes_3d, gt_labels_3d
    ):
        if (
            self.training
            and self.train_cfg.get("disable_pretrain", False)
            and not self.runtime_info.get("enable_detection", False)
        ):
        ...
        if cfg.get("add_gt_fg_points", False):
            ...
            for i, points in enumerate(point_list):
                ...
                if not gt_mask.any() or len(points) == 0:
                    gt_fg_mask_list.append(
                        gt_mask.new_zeros(len(points), dtype=torch.bool)
                    )
                    continue
    ...
    def group_sample(self, dict_to_sample, offset):
        ...
        grouped_score = self.gather_group_by_names(
            seg_scores[:, :-1]
        )  # without background score
        ...
        for i in range(num_groups):
            ...
            assert torch.isclose(
                offset_weight.sum(1), offset_weight.new_ones(len(offset_weight))
            ).all()
...
class ClusterAssigner(torch.nn.Module):
    ...
    def forward(
        ...
        cluster_inds_list, valid_mask_list = multi_apply(
            self.forward_single_class,
            points_list,
            batch_idx_list,
            self.class_names,
            origin_points,
    ...
    def forward_single_class(self, points, batch_idx, class_name, origin_points):
        ...
        coors = torch.div(
            points - pc_range[None, :3], voxel_size[None, :], rounding_mode="floor"
        ).int()
        # coors = coors[:, [2, 1, 0]] # to zyx order
        coors = torch.cat([batch_idx[:, None], coors], dim=1)
        ...
        sampled_centers, voxel_coors, inv_inds = scatter_v2(
        ...
        if self.training:
            cluster_inds = find_connected_componets(
                sampled_centers, voxel_coors[:, 0], dist
            )
        else:
            if self.gpu_clustering[1]:
                cluster_inds = find_connected_componets_gpu(
                    sampled_centers, voxel_coors[:, 0], dist
                )
            else:
                cluster_inds = find_connected_componets_single_batch(
                    sampled_centers, voxel_coors[:, 0], dist
                )
...
class HybridAssigner(torch.nn.Module):
    ...
    def forward_ssg(self, points, batch_idx, class_name, origin_points):
        ...
        voxels, voxel_coors, inv_inds = scatter_v2(
            points, coors, mode="avg", return_inv=True
        )
