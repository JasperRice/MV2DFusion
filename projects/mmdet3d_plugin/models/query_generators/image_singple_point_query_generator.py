...
class ImageSinglePointQueryGenerator(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        if init_cfg is None:
            ...
            if self.with_center:
                self.init_cfg += [
                    dict(type="Normal", std=0.001, override=dict(name="fc_center"))
                ]
            if self.with_size:
                self.init_cfg += [
                    dict(type="Normal", std=0.001, override=dict(name="fc_size"))
                ]
            if self.with_heading:
                self.init_cfg += [
                    dict(type="Normal", std=0.001, override=dict(name="fc_heading"))
                ]
            if self.with_attr:
                self.init_cfg += [
    ...
    def build_branch(self):
        ...
        self.size_convs, self.size_fcs, self.size_last_dim = self._add_conv_fc_branch(
        ...
        self.attr_convs, self.attr_fcs, self.attr_last_dim = self._add_conv_fc_branch(
    ...
    def get_box_params(self, bboxes, intrinsics, extrinsics, min_bbox_size=4):
        intrinsic_list = []
        extrinsic_list = []
        for img_id, (bbox, intrinsic, extrinsic) in enumerate(
            zip(bboxes, intrinsics, extrinsics)
        ):
    ...
    def center2lidar(self, center_pred, intrinsic, extrinsic):
        center_img = torch.cat(
            [center_pred[:, :2] * center_pred[:, 2:3], center_pred[:, 2:3]], dim=1
        )
        center_img_hom = torch.cat(
            [center_img, center_img.new_ones([center_img.shape[0], 1])], dim=1
        )  # [num_rois, 4]
    ...
    def forward(self, x, proposal_list, img_metas, debug_info=None, **kwargs):
        ...
        center_pred, return_feats = self.get_prediction(
            roi_feat, intrinsics, extrinsics, extra_feats, return_feats
        )
