...
class PointCloudQueryGenerator(BaseModule):
    ...
    def forward(
        ...
        assert (
            not any(x.requires_grad for x in query_xyz)
            and not any(x.requires_grad for x in query_pred)
            and not any(x.requires_grad for x in query_cat)
            and not any(x.requires_grad for x in query_feat)
        )
        ...
        bev_indices, bev_indices_inv = lidar_indices[:, [0, 2, 3]].unique(
            dim=0, return_inverse=True
        )  # [n_v, (bid, y, x)]
        bev_lidar_feat = self.pre_bev_embed(lidar_feat) + lidar_feat
        bev_feat = torch_scatter.scatter(
            bev_lidar_feat, bev_indices_inv, dim=0, reduce="mean"
        )
        bev_xyz = (bev_indices[:, [2, 1]] + 0.5) * voxel_size[None, :2] + pc_range[
            None, :2
        ]
        lidar_feat = bev_feat
        ...
        if self.pts_use_cat:
            query_feat_w_pred = [
                x + self.pts_cat_embed(cat)
                for x, cat in zip(query_feat_w_pred, query_cat)
            ]
        ...
        lidar_xyz = (lidar_xyz[:, :2] - head_pc_range[:2]) / (
            head_pc_range[3:5] - head_pc_range[:2]
        )
        ...
        lidar_feat_in = (
            lidar_feat.new_zeros([batch_size, max_size, feat_size])
            + self.empty_embed.weight
        )
        for b in range(batch_size):
        ...
        pad_size = min(max_size, self.empty_pos.weight.size(0))
        query_pos[:, -pad_size:] = (
            self.empty_pos.weight[:pad_size, :3]
            * (head_pc_range[3:6] - head_pc_range[0:3])
            + head_pc_range[0:3]
        )
        for b in range(batch_size):
