...
def topk_gather(feat, topk_indexes):
    if topk_indexes is not None:
        ...
        feat = torch.gather(
            feat, 1, topk_indexes.expand(*topk_indexes.shape[:2], *feat.shape[2:])
        )
    return feat
...
def locations(features, stride, pad_h, pad_w):
    ...
    shifts_x = (
        torch.arange(0, stride * w, step=stride, dtype=torch.float32, device=device)
        + stride // 2
    ) / pad_w
    shifts_y = (
...
def transform_reference_points(
    ...
    if not translation:
        matrix[..., :3, 3] = 0.0
    reference_points = (matrix.unsqueeze(1) @ reference_points.unsqueeze(-1)).squeeze(
        -1
    )[..., :3]
