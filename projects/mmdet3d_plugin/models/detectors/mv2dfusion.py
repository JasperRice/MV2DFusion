...
from mmdet.models import (DETECTORS, build_detector, build_head, build_neck,
                          build_roi_extractor)
...
def bbox3d2result(
    bboxes, scores, labels, obj_idxes=None, track_scores=None, attrs=None
):
    result_dict = dict(
...
class MV2DFusion(MVXTwoStageDetector):
    ...
    def __init__(
        ...
        super(MV2DFusion, self).__init__(
            pts_voxel_layer,
            pts_voxel_encoder,
            pts_middle_encoder,
            pts_fusion_layer,
    ...
    def forward_roi_head_train(self, imgs, feats, img_metas, gt_bboxes, gt_labels):
        ...
        if not valid_inds.any():
            ...
            losses = {
                k: ([x * 0 for x in v] if isinstance(v, (list, tuple)) else v * 0)
    ...
    def process_2d_gt(self, gt_bboxes, gt_labels, device):
        return [
            torch.cat(
                [
                    bboxes.to(device),
                    torch.ones([len(labels), 1], dtype=bboxes.dtype, device=device),
                    labels.unsqueeze(-1).to(bboxes.dtype),
                ],
                dim=-1,
            ).to(device)
    ...
    def complement_2d_gt(self, detections, gts, thr=0.6, with_id=False):
        ...
        if with_id:
            complement_gts = torch.cat(
                [complement_gts, torch.zeros_like(complement_gts[..., :1]) - 1], dim=-1
            )
        ...
        detections = [
            torch.cat(
                [
                    ...
                    for label_id, boxes in enumerate(res)
                ],
                dim=0,
            )
            for res in results
    ...
    def extract_roi_feats(self, feats_det, rois, **data):
        roi_feats = self.img_roi_extractor(
            feats_det[: self.img_roi_extractor.num_inputs], rois
        )
        return roi_feats
    ...
    def forward_pts_train(
        ...
        **data
        ...
        imgs_det, feats_det, img_metas_det, imgs_shape = self.prepare_detection_data(
            img_metas, **data
        )
        ...
        if not requires_grad:
            ...
            if self.with_img_roi_head:
                losses_det2d = self.forward_roi_head_train(
                    imgs_det, feats_det, img_metas_det, gt_bboxes, gt_labels
                )

            self.eval()
            with torch.no_grad():
                dets2d, dets = self.forward_roi_head(imgs_det, feats_det, img_metas_det)
                dets_gt = self.process_2d_gt(
                    sum(gt_bboxes, []), sum(gt_labels, []), imgs_det.device
                )
                assert len(dets) == (len(dets_gt))
                if self.use_2d_proposal:
                    dets = sum([x for x in data["proposals"]], [])
                dets = [
                    self.complement_2d_gt(
                        det,
                        det_gt,
                    )
            ...
            n_rois_per_batch = [
                sum(n_rois_per_view[i * V : (i + 1) * V]) for i in range(B)
            ]

            dyn_query, dyn_feats = self.img_query_generator(
                roi_feats,
                dets,
                img_metas_det,
            ...
            if self.gt_mono_loss:
                ...
                n_rois_per_batch_gt = [
                    sum(n_rois_per_view_gt[i * V : (i + 1) * V]) for i in range(B)
                ]
                _, dyn_feats = self.img_query_generator(
                    roi_feats_gt,
                    dets_gt,
                    img_metas_det,
            ...
            out_dict = self.pts_backbone.forward_train(
                data["pts_feats"], img_metas, fsd_gt_bboxes_3d, fsd_gt_labels_3d
            )
            pts_feat, pts_pos, pts_query_feat, pts_query_center = (
                self.pts_query_generator(
                    out_dict["voxel_feats"],
                    out_dict["voxel_coors"],
                    out_dict["voxel_xyz"],
                    out_dict["query_feats"],
                    out_dict["query_xyz"],
                    out_dict["query_pred"],
                    out_dict["query_cat"],
                    B,
            ...
            outs = self.fusion_bbox_head(
                img_metas,
                dyn_query=dyn_query,
                dyn_feats=dyn_feats_pred,
        ...
        if return_losses:
            ...
            if dyn_feats is not None:
                ...
                if dyn_feats.get("cls_scores", None) is not None:
                    ...
                    gt_bboxes_3d_ = [
                        torch.cat((b.gravity_center, b.tensor[:, 3:]), dim=1)
                        .to(imgs_det.device)
                        .clone()
                    ...
                    for x in gt_bboxes_3d_:
                        x[:, 6:] = 0
                    loss_cls, loss_bbox = self.fusion_bbox_head.loss_single(
                        cls_scores, bbox_preds, gt_bboxes_3d_, gt_labels_3d
                    )
    ...
    def forward_train(
        ...
        **data
        ...
        rec_img_feats, rec_img_feats_for_det = self.extract_img_feat(
        ...
        if T - self.num_frame_backbone_grads > 0:
            self.eval()
            with torch.no_grad():
                prev_img_feats, prev_img_feats_for_det = self.extract_img_feat(
            ...
            data["img_feats_for_det"] = [
                torch.cat(
                    [
                        prev.view(B, prev_T, V, *prev.shape[1:]),
                        rec.view(B, rec_T, V, *rec.shape[1:]),
                    ],
                    dim=1,
                )
            ...
            data["img_feats_for_det"] = GroupedItems(
                [x.view(B, T, V, *x.shape[1:]) for x in rec_img_feats_for_det]
            )
    ...
    def simple_test_pts(self, img_metas, **data):
        ...
        if sum([len(p) for p in dets]) == 0:
            proposal = torch.tensor(
                [[0, 50, 50, 100, 0, 1]], dtype=dets[0].dtype, device=dets[0].device
            )
            dets = [proposal] + dets[1:]
        rois = bbox2roi(dets)
        ...
        bbox_results = [bbox3d2result(*bbox) for bbox in bbox_list]
        return bbox_results
    ...
    def simple_test(self, img_metas, **data):
        ...
        data["img_feats_for_det"] = [
            x.view(B, V, *x.shape[1:]) for x in img_feats_for_det
        ]
