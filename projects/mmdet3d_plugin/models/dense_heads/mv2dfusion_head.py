...
from projects.mmdet3d_plugin.models.utils.misc import (
    MLN, SELayer_Linear, memory_refresh, topk_gather,
    transform_reference_points)
from projects.mmdet3d_plugin.models.utils.positional_encoding import (
    nerf_positional_encoding, pos2posemb1d, pos2posemb3d)
...
class MV2DFusionHead(AnchorFreeHead):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(MV2DFusionHead, self).__init__(
    ...
    def pre_update_memory(self, data):
        ...
        if self.memory_embedding is None or data["timestamp"].size(
            0
        ) != self.memory_embedding.size(0):
            ...
            self.memory_query_mask = x.new_zeros(
                B, self.memory_len, 1, dtype=torch.bool
            )
            ...
            self.memory_query_mask = memory_refresh(
                self.memory_query_mask[:, : self.memory_len], x.bool()
            )
            self.memory_instance_inds = memory_refresh(
                self.memory_instance_inds[:, : self.memory_len], x, value=-1
            )
    ...
    def post_update_memory(
        ...
        if query_mask is not None:
            ...
            self.memory_query_mask = torch.cat(
                [query_mask, self.memory_query_mask], dim=1
            )
        if instance_inds is not None:
            instance_inds = torch.gather(instance_inds, 1, topk_indexes[..., 0])
            self.memory_instance_inds = torch.cat(
                [instance_inds, self.memory_instance_inds], dim=1
            )

    @staticmethod
    def transform3d(pose, coords3d):
        coords3d = torch.cat(
            [coords3d, torch.ones_like(coords3d[..., 0:1])], dim=-1
        )  # B, ..., 4
    ...
    def get_box_info(bbox_preds):
        bbox_x, bbox_y, bbox_w, bbox_l, bbox_o = (
            bbox_preds[..., 0],
            bbox_preds[..., 1],
            bbox_preds[..., 3],
            bbox_preds[..., 4],
        ...
        pc0 = torch.stack(
            [
                bbox_x + cos * bbox_l / 2 + sin * bbox_w / 2,
                bbox_y + sin * bbox_l / 2 - cos * bbox_w / 2,
            ],
            dim=-1,
        )
        pc1 = torch.stack(
        ...
        return (
            xyxyo,
            torch.stack([bbox_w, bbox_l, bbox_h], dim=-1),
    ...
    def gen_dynamic_query(self, static_query, dynamic_query, dynamic_query_feats=None):
        ...
        for b in range(B):
            ...
            if dynamic_query_feats is not None:
                query_feats[b, : dyn_q.size(0)] = dynamic_query_feats[b][
                    : dyn_q.size(0)
                ]
    ...
    def forward(
        ...
        **data,
        ...
        mln_input = torch.cat(
            [intrinsics[..., 0, 0:1], intrinsics[..., 1, 1:2], extrinsics.flatten(-2)],
            dim=-1,
        )
        ...
        spatial_flatten_img = torch.as_tensor(
            spatial_flatten_img, dtype=torch.long, device=mlvl_feats[0].device
        )
        level_start_index_img = torch.cat(
            (
                spatial_flatten_img.new_zeros((1,)),
                spatial_flatten_img.prod(1).cumsum(0)[:-1],
            )
        )
        ...
        reference_points, query_coords, query_probs, query_feats, query_mask = (
            self.gen_dynamic_query(
                self.reference_points.weight,
                dyn_query,
                dyn_feats.get("query_feats", None),
            )
        )
        ...
        query_mask = torch.cat(
            [torch.ones_like(pts_ref[..., 0]).bool(), query_mask], dim=1
        )
        ...
        if attn_mask is None:
            attn_mask = torch.zeros(
                (tgt_size, src_size), dtype=torch.bool, device=reference_points.device
            )
        pad_size = attn_mask.size(0) - tgt_size
        if mask_dict is not None:
            assert pad_size == mask_dict["pad_size"]
        attn_mask = attn_mask.repeat(B, 1, 1)
        tgt_query_mask = torch.cat(
            [query_mask, self.memory_query_mask[:, : self.num_propagated, 0]], dim=1
        )
        ...
        tgt = torch.cat(
            [tgt.new_zeros((B, pad_size, self.embed_dims)), pts_tgt, tgt], dim=1
        )
        pad_query_feats = query_feats.new_zeros(
            [B, pad_size + self.num_query, self.embed_dims]
        )
        pts_query_feat = self.pts_query_embed(pts_query_feat)
        pad_query_feats[:, pad_size : pad_size + num_query_pts] = pts_query_feat
        pad_query_feats[:, pad_size + num_query_pts : pad_size + self.num_query] = (
            query_feats
        )
        ...
        dyn_q_mask_img = dyn_q_mask[
            :, pad_size + num_query_pts : pad_size + self.num_query
        ]
        ...
        all_cls_scores[:, :, pad_size:][:, ~tgt_query_mask] = (
            -40 + all_cls_scores[:, :, pad_size:][:, ~tgt_query_mask] * 0
        )
        all_bbox_preds[:, :, pad_size:][:, ~tgt_query_mask] = (
            0 + all_bbox_preds[:, :, pad_size:][:, ~tgt_query_mask] * 0
        )
        ...
        if len(ops) > 0:
            ...
            score_bev = (
                all_cls_scores[-1, :, pad_size:].sigmoid().max(-1).values.clone()
            )
            ...
            nms_tgt_query_mask &= (
                all_cls_scores[-1, :, pad_size:].sigmoid().max(-1).values > score_thr
            )
