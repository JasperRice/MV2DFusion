...
class AV2LoadMultiViewImageFromFiles(object):
    ...
    def __call__(self, results):
        ...
        img = [
            mmcv.imread(name, self.color_type).astype(np.float32) for name in filename
        ]
        ...
        for i in range(N):
            ...
            if H > W:
                    ...
                    if len(gt_bboxes) != 0:
                        gt_bboxes, centers2d, gt_labels, depths = (
                            self._filter_invisible(
                                img, gt_bboxes, centers2d, gt_labels, depths
                            )
                        )
                ...
                results["intrinsics"][i][:3, :3] = (
                    ida_mat @ ida_mat_f @ results["intrinsics"][i][:3, :3]
                )
...
class AV2PadMultiViewImage:
    ...
    def _pad_img(self, results):
        ...
        if self.size == "same2max":
            ...
            padded_img = [
                mmcv.impad(img, shape=max_shape, pad_val=self.pad_val)
                for img in results["img"]
            ]
            with_depthmap = "depthmap" in results.keys()
            if with_depthmap:
                results["depthmap"] = [
                    mmcv.impad(depthmap, shape=max_shape, pad_val=self.pad_val)
                    for depthmap in results["depthmap"]
                ]
...
class ResizeCropFlipRotImage:
        ...
        for i in range(N):
            ...
            if self.with_2d:  # sync_2d bbox labels
                if "gt_bboxes" in results:
                    ...
                    gt_inds = (
                        None
                        if "instance_inds_2d" not in results
                        else results["instance_inds_2d"][i]
                    )
                    if len(gt_bboxes) != 0:
                        gt_bboxes, centers2d, gt_labels, depths, gt_inds = (
                            self._bboxes_transform(
                                gt_bboxes,
                                centers2d,
                                gt_labels,
                    ...
                    if len(gt_bboxes) != 0 and self.filter_invisible:
                        gt_bboxes, centers2d, gt_labels, depths, gt_inds = (
                            self._filter_invisible(
                                gt_bboxes, centers2d, gt_labels, depths, gt_inds=gt_inds
                            )
                        )
                ...
                if "proposals" in results:
                    ...
                    if len(proposals) > 0:
                        proposals = self._proposals_transform(
                            proposals, resize, crop, flip
                        )
                        if self.filter_invisible:
                            fH, fW = self.data_aug_conf["final_dim"]
                            proposal_boxes = proposals[..., :4]
                            xy_max = np.minimum(
                                proposal_boxes[..., 2:4], [fW - 1, fH - 1]
                            )
    ...
    def _bboxes_transform(
        self, bboxes, centers2d, gt_labels, depths, resize, crop, flip, gt_inds=None
    ):
        assert len(bboxes) == len(centers2d) == len(gt_labels) == len(depths)
        fH, fW = self.data_aug_conf["final_dim"]
        bboxes = bboxes * resize
...
class GlobalRotScaleTransImage:
    ...
    def _trans_xyz(self, results, trans):
        ...
        for view in range(num_view):
            ...
            results["extrinsics"][view] = (
                torch.tensor(results["extrinsics"][view]).float() @ trans_mat_inv
            ).numpy()
    ...
    def _rotate_bev_along_z(self, results, angle):
        ...
        for view in range(num_view):
            ...
            results["extrinsics"][view] = (
                torch.tensor(results["extrinsics"][view]).float() @ rot_mat_inv
            ).numpy()
    ...
    def _scale_xyz(self, results, scale_ratio):
        ...
        for view in range(num_view):
            ...
            results["extrinsics"][view] = (
                torch.tensor(results["extrinsics"][view]).float() @ scale_mat_inv
            ).numpy()
...
class BEVGlobalRotScaleTrans(GlobalRotScaleTransImage):
    def __call__(self, results):
        ...
        if self.reverse_angle:
            rot_angle = rot_angle * -1
        points, _ = results["gt_bboxes_3d"].rotate(np.array(rot_angle), points)
