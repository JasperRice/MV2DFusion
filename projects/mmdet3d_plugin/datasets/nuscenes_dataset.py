...
class CustomNuScenesDataset(NuScenesDataset):
    ...
    def load_proposals(self):
        ...
        assert len(set(total_ann_ids)) == len(
            total_ann_ids
        ), f"Annotation ids in '{self.proposal_file}' are not unique!"
    ...
    def get_proposals(self, img_info, proposal_info):
        ...
        if self.proposal_nms is not None and len(proposal_bboxes) > 0:
            ...
            class_agnostic = self.proposal_nms.get(
                "class_agnostic", nms_cfg.get("class_agnostic")
            )
            ...
            proposal_bboxes, proposal_scores, proposal_labels = (
                proposal_bboxes[inds],
                proposal_scores[inds],
                proposal_labels[inds],
            )
        ...
        proposals = np.concatenate(
            [proposal_bboxes, proposal_scores[:, None], proposal_labels[:, None]],
            axis=-1,
            dtype=np.float32,
        )
        return proposals
    ...
    def _set_sequence_group_flag(self):
        ...
        if self.seq_split_num != 1:
            if self.seq_split_num == "all":
                ...
                for curr_flag in range(len(bin_counts)):
                    curr_sequence_length = np.array(
                        list(range(0, bin_counts[curr_flag], self.seq_length))
                        + [bin_counts[curr_flag]]
                    )
