...
def main():
    ...
    data_infos = dataset.data_infos_tracking
    ...
    for sample_idx, sample_token in enumerate(sample_tokens):
        ...
        pc = raw_data["points"][0].data.numpy()[:, :3]
        ...
        l2g = e2g @ l2e
        new_pcs = np.concatenate((pc, np.ones(pc.shape[0])[:, np.newaxis]), axis=1)
        ...
        if scene_token != last_scene_token and last_scene_token is not None:
            print("Making Videos")
            show_dir = os.path.join(args.show_dir, last_scene_token)
            ...
            make_videos(
                show_dir,
                fig_names,
                "{}-video.mp4".format(last_scene_token),
                args.show_dir,
            )
