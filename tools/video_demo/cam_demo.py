...
from projects.tracking_plugin.visualization import \
    NuscenesTrackingBox as TrackingBox
        ...
        if sensor_modality in ["lidar", "radar"]:
            ...
            boxes = [
                TrackingBox(
                    ...
                    tracking_id=record["tracking_id"].split("-")[-1],
                )
                for record in pred_data["results"][sample_toekn]
                if record["tracking_score"] > 0.4
            ]
...
def parse_args():
    ...
    parser.add_argument(
        "--data_infos_path",
        type=str,
        default="./data/nuscenes/tracking_forecasting-mini_infos_val.pkl",
    )
...
if __name__ == "__main__":
    ...
    for video_index, scene_token in enumerate(scene_tokens):
        ...
        indexes = sorted(
            [int(fname.split("_")[0]) for fname in fig_names if fname.endswith("png")]
        )
