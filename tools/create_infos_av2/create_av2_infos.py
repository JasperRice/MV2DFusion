...
import pandas as pd
import torch
from av2.datasets.sensor.constants import AnnotationCategories, RingCameras
...
dataset_dir = Path("./data/argo/sensor")  # replace with absolute path
out_dir = Path("./data/argo/converted")
...
def create_av2_infos(dataset_dir, split, out_dir):
    ...
    )  # 每个split包含的所有lidar帧路径
    records = [
        convert_path_to_named_record(p) for p in paths
    ]  # 每帧信息：split，log_id, sensor_name, timestamp_ns
    sensor_caches = pd.DataFrame(records)  # pandas形式
    sensor_caches.set_index(
    ...
    for i in tqdm(range(len(sensor_caches))):
        ...
        for cam_name in CAMERAS:
            ...
            camera_model = PinholeCamera.from_feather(
                log_dir=log_dir, cam_name=cam_name
            )
            intrinsics_path = log_dir / "calibration" / "intrinsics.feather"
            intrinsics_df = io_utils.read_feather(intrinsics_path).set_index(
                "sensor_name"
            )
        ...
        if split != "test":
            ...
            gt3d_infos = get_gt3d_data(
                dataset_dir=dataset_dir,
                split=split,
                log_id=log_id,
                timestamp_ns=lidar_timestamp_ns,
            )
            gt2d_infos = get_gt2d_data(
                camera_models,
                cam_infos,
...
def _load_annotations(split, log_id, timestamp_ns):
    ...
    cuboids = list(
        filter(lambda x: x.timestamp_ns == timestamp_ns, cuboid_list.cuboids)
    )
...
def get_gt3d_data(dataset_dir, split, log_id, timestamp_ns):
    ...
    gt_boxes = torch.cat(
        (cuboid_params[:, :-4], torch.as_tensor(rot).unsqueeze(1)), dim=-1
    )

    names = [label_class for label_class in annotations["category"].to_numpy()]
...
def get_gt2d_data(
    ...
    if synchronized_imagery is not None:
        ...
        for _, cam in synchronized_imagery.items():
            ...
            city_SE3_ego_cam_t = timestamp_city_SE3_ego_dict[
                cam_infos[cam.cam_name]["cam_timestamp_ns"]
            ]  # ego2glo at cam timestamp
            city_SE3_ego_lidar_t = timestamp_city_SE3_ego_dict[
                lidar_timestamp_ns
            ]  # ego2glo at lidar timestamp
            centers3d = annotations.xyz_center_m  # N, 3
            categories = annotations.categories
            categories_idx = np.array([label_class for label_class in categories])
            ...
            if city_SE3_ego_cam_t is not None and city_SE3_ego_lidar_t is not None:
                ...
                uv_corners, cuboids_vertices_cam, _ = (
                    cam.project_ego_to_img_motion_compensated(
                ...
                cuboids_vertices_ego = cuboids_vertices_ego.reshape(-1, D)
                uv_corners, cuboids_vertices_cam, _ = cam.project_ego_to_img(
                    cuboids_vertices_ego
                )
            ...
            for i in range(cuboids_vertices_cam.shape[0]):
                ...
                final_coords = post_process_coords(
                    uv_corner, imsize=(cam.width_px, cam.height_px)
                )
